...
const EvaluationsPage = () => {
    ...
    const allEvaluated = rows.length > 0 &&
        rows.every((row) => metaKeys.every((meta) => row.metas[meta] && row.metas[meta] !== "MANA"));
    const isRowComplete = (row) => metaKeys.every((meta) => row.metas[meta] && row.metas[meta] !== "MANA");
    ...
    const sendStudentDigest = async (studentId) => {
        if (!selectedClassId) {
            return;
        }
        setError(null);
