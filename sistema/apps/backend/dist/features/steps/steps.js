...
Given("a student named {string} with cpf {string} and email {string} exists", async function (name, cpf, email) {
    const response = await this.api.post("/students").send({ name, cpf, email });
    assert.equal(response.status, 201);
    this.studentsByName.set(name, response.body);
});
When("I create a student named {string} with cpf {string} and email {string}", async function (name, cpf, email) {
...
Then("the student is created", function () {
    assert.ok(this.lastResponse);
    assert.equal(this.lastResponse?.status, 201);
});
Then("the students list includes {string}", async function (name) {
    const response = await this.api.get("/students");
    assert.equal(response.status, 200);
    assert.ok(response.body.some((s) => s.name === name));
});
Then('the request fails with status {int}', function (status) {
    assert.ok(this.lastResponse);
    assert.equal(this.lastResponse?.status, status);
});
Given("a class with topic {string} year {int} semester {int} exists", async function (topic, year, semester) {
    const response = await this.api.post("/classes").send({ topic, year, semester });
    assert.equal(response.status, 201);
    this.classesByTopic.set(topic, response.body);
});
When("I create a class with topic {string} year {int} semester {int}", async function (topic, year, semester) {
...
Then("the class is created", function () {
    assert.ok(this.lastResponse);
    assert.equal(this.lastResponse?.status, 201);
});
When("I enroll {string} into {string}", async function (studentName, topic) {
    ...
    this.lastResponse = await this.api.post(`/classes/${classEntity.id}/students/${student.id}`);
});
Then("the class roster for {string} includes {string}", async function (topic, studentName) {
    const classEntity = this.classesByTopic.get(topic);
    assert.ok(classEntity);
    const response = await this.api.get(`/classes/${classEntity.id}/students`);
    assert.equal(response.status, 200);
    assert.ok(response.body.some((s) => s.name === studentName));
});
Given("{string} is enrolled in {string}", async function (studentName, topic) {
    const student = this.studentsByName.get(studentName);
    const classEntity = this.classesByTopic.get(topic);
    assert.ok(student && classEntity);
...
When("I set meta {string} to {string} for {string} in {string}", async function (meta, status, studentName, topic) {
    const student = this.studentsByName.get(studentName);
    const classEntity = this.classesByTopic.get(topic);
    assert.ok(student && classEntity);
...
Then("the evaluations table for {string} shows {string} has {string} = {string}", async function (topic, studentName, meta, status) {
    const classEntity = this.classesByTopic.get(topic);
    assert.ok(classEntity);
    const response = await this.api.get(`/classes/${classEntity.id}/evaluations`);
    assert.equal(response.status, 200);
    const row = response.body.rows.find((r) => r.studentName === studentName);
    assert.ok(row);
    assert.equal(row.metas[meta], status);
});
When("I run the daily digest job", async function () {
    const response = await this.api.post("/jobs/send-daily-digests").send({});
    assert.equal(response.status, 200);
    this.digestEmails = response.body.sent;
});
Then("one digest email is sent to {string}", function (email) {
    const matches = this.digestEmails.filter((e) => e.to === email);
    assert.equal(matches.length, 1);
});
Then("the digest includes {string} and {string}", function (metaA, metaB) {
